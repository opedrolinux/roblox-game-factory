...
| 3 | **A1 — the harness refuses a gating phase that touches the game nowhere** | **PARTLY — the forgery is CLOSED, the self-parenting subject is NOT.** The ledger hole is fixed and pinned: a private file-local `MINT` table now stamps only harness-built readers, an author-built `{ channel = "world", read = ... }` is downgraded to `opaque` rather than believed, and `effectiveChannel` may no longer PROMOTE an unminted read to `reply`/`world` — it can only demote and name a `saved` read. So `read = function() Harness.countBaseParts(); return myLocal end` no longer earns game contact. Proven against the REAL template by `t25_forgery_spec` (10 checks), and proven able to fail: re-inserting the promotion branch flips attack A back to `ok=true [world=1]` and reds the spec. **Self-parenting is now CLOSED too (2026-08-03).** A `world` delta whose act made no harness-owned game action (`invokeAs`, `embody`/`moveBody`) is refused by name: nothing but the phase is known to have touched the world in that window. Pinned by `t25_forgery_spec` attack D, which runs on the UNRELAXED template, and paired with a positive control (routing the same act through `moveBody` clears it) so a blanket refusal could not score as a pass. The shipped `example-delta` — which had this exact shape — was rewritten to drive its act through `Harness.embody` | done |
| 4 | **A2 — the ROOT PATTERN cannot be laundered through `probe:delta`** | **PARTLY.** The half that rode on the forged channel is closed with row 3 — `probe:governs` can no longer be satisfied by a discarded `countBaseParts()` call, because the governing reader must now be MINTED. **The aggregate is now CLOSED too (2026-08-03).** Channels are tallied PER SUBJECT as well as phase-wide, and a subject asserted only against the save file is refused BY NAME even when another subject in the same phase reached the game. Falsified both ways against the real template: with the rule disabled the laundering fixture passes green, reporting `2 subject(s), 2 delta(s) asserted [world=1 saved=1]; 2 assertion(s) passed` — the root pattern with an alibi — and with it enabled the same fixture reds naming `capacity-upgrade`. Pinned by `t25_forgery_spec` attack C plus a control proving a subject with its own paired reader still passes | done |
...
| 6 | **A9 — a hang cannot publish a stale green** | **HALF.** A deadline exists and works (driven directly by substituting the budget to `-1`: *"the run budget is spent: 0.0s elapsed of a -1s budget"*), **but it is read BEFORE each phase and is not a watchdog** — the template says so itself at line 1775. A body that never returns is not interrupted: `run=green bodyRanToCompletion=true`, and the A9 scenario as written still produces **no sentinel line**. **The reader side is now CLOSED (2026-08-03)**, and it caught a live false green on the way in. `readT25` now delegates to `t25-provenance.luau`, which refuses an artifact that (a) carries no `provenance` block, (b) records `scriptSha256` absent or `"unknown"`, (c) records a hash that does not match the harness on disk, or (d) predates the newest mtime under `src/`. **Measured, not assumed:** the committed reader, run against `games/collect-sim`'s real 2026-08-01 artifact, returned `ready: true, highestGreen: "T2.5", "scene-verified-T2.5 — geometry, wiring and server effects checked"` from an artifact with NO provenance at all, while the harness and the game source had both changed under it. The new reader returns `t25Evidence: "red"` with `t25StaleReason` naming the missing block. Pinned by `t25_provenance_spec` (13 checks), including two positive controls. **What remains is the in-process watchdog only:** a body that never returns is still not interrupted, and the capture recipe still does not check `run-in-roblox`'s exit status — but a hang can no longer publish a stale green, because the stale artifact is now refused at the reader | the watchdog needs a scheduler property nobody here has measured; the exploitable half is closed |
...
**All nine adversarial attacks on the T2.5 harness are now refused, and A9's exploitable half with them (2026-08-03).** The channel ledger is no longer forgeable, the per-phase aggregate is now per-subject, a phase can no longer assert over instances it parented itself, and a stale artifact can no longer credit a tree it never saw. What survives is narrower and stated as such: an in-process watchdog for a body that never returns. The harness
...
| 3 | ~~Add provenance to `readT25`~~ **DONE 2026-08-03** — `t25-provenance.luau`, pinned by `t25_provenance_spec` (13 checks). It caught a live false green on `games/collect-sim` the moment it was wired in | — | §7 row 6 (reader half) | — |
...
| 5 | ~~The one design hole~~ **DONE 2026-08-03.** Per-SUBJECT channels (attack C, falsified both ways) and the self-parenting rule (attack D, with a positive control). The mint half was already done. `example-delta` rewritten, since it shipped the defect | — | §7 rows 3, 4 | — |
