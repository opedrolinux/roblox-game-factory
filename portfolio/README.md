...
| Deep Reach | deep-reach | **verified-local-T1** | **not the human yet** — the adversarial review was CUT SHORT by the account's monthly spend limit and `grade.js` never ran; both are owed before the human gate. Then T2 -> T2.5 -> T2.7, all `unrun`. | Second game, and the first built end-to-end by the supervised `build-game` loop. Spec: `specs/deep-reach.md`. All 9 features built and merged on `staging/deep-reach`; gauntlet **6/6 GREEN, 922/922 lune, reachability 0 FAIL** (28 at the contract pass — each feature cleared exactly the subjects it owned). **Never booted in Roblox**: T2/T2.5/T2.7 unrun, so this is logic-under-Lune plus static gates only. Full handoff: `docs/build-records/deep-reach/HANDOFF.md`. **The build's most valuable finding was about the factory, not the game:** `build-features.js` aggregated real bugs from the bug-hunter critic ONLY, though all three critics carry `realBugsFound` — so features with genuine defects routed to `needs-review` (a park) instead of `bug-found` (the auto-fix loop), for four batches. 26 findings recovered from the run journals, 19 still open, all 19 fixed; the very next batch the repaired aggregation caught 3 more bugs, 2 of them from the coverage critic. **A second gate defect of the same class:** the adversarial review counted rounds in which every hunter DIED as *dry* rounds, so a loop-until-dry security sweep terminated because it stopped being able to look and returned the shape of convergence — fixed, and it now reports its own coverage. **Real defects the gates caught:** the offline claim popup gave up 28s before a contended load can finish (permanent loss of the away window, the client-side door no server-side rung can see); a free depth tier out-earned the paywalled one 14x vs 12x one day in five; a real-money purchase could emit its revenue event twice, and separately zero times; `EconomyService.earn` credited a balance and then refused. **Two contract amendments, both reversible:** A12 (the daily streak ladder was unclimbable at a 24h cadence — six of seven rungs dead content) and A13 (`PlayerView` was replicating three fields nothing read, through a `stats` wholesale copy that was the one hole in the allowlist). **Monetization is INERT in the shipped build** — every asset id is 0 because they come from a published place; the build now says so loudly at boot, but no pass grants, no receipt is recognised, and `purchase` has no reachable emit point until a human fills them. **Pacing is unverified by machine** (D2): a coarse order-of-magnitude band only. |
